...
export const postApplication = catchAsyncError(async (req, res, next) => {
  ...
  if (!allowedFormats.includes(resume.mimetype)) {
    return next(
      new ErrorHandler("Invalid file type. Please upload a PNG/JPEG/WEBP file.", 400)
    );
  }
  ...
  if(!user){
    return next(
      new ErrorHandler("Invalid Email, First register on the portal then Apply Well!.", 400)
    )
